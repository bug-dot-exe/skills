...
### Cross-Platform Code Audit (Windows K8s Nodes)

K8s on Windows is under-tested. Focus on platform-specific code paths:
```bash
...
# Windows-specific volume handling may have different sanitization
# CVE-2023-5528: insufficient input sanitization in in-tree storage plugin
# Test: volume names with special characters, path traversal in subPath
```

### Controller Reconciler DoS via Malformed CRDs

Audit every Custom Resource Definition for controller crash vectors:
```bash
# List all CRDs
kubectl get crd -o json | jq '.items[].metadata.name'
# For each CRD, create a resource with edge-case values:
# - Non-existing references (volumeSnapshotClassName that doesn't exist)
# - Empty string values where non-empty is expected
# - Extremely long strings
# - Special characters in name/namespace fields
```
If the controller crashes or enters a panic loop on malformed input, it stops reconciling all resources.

### Sanitizer/Handler List Drift

When a K8s component has parallel registries for "supported" vs "validated" items:
```bash
# Ingress annotation injection example
# List all supported annotations vs validated annotations
# Annotations that are processed but not validated allow injection
kubectl annotate ingress test-ingress \
  nginx.ingress.kubernetes.io/permanent-redirect='http://evil.tld' --overwrite
```
The gap between "supported by the handler" and "validated by the admission webhook" is the exploit surface.

### Wildcard Endpoint Authorization Bypass

RBAC-protected APIs that offer both specific and wildcard endpoints:
```bash
# Test: does /resources/* bypass /resources/{id} authorization?
kubectl get pods --all-namespaces  # List endpoint
kubectl get pod specific-pod -n restricted  # Single endpoint
# Some implementations check RBAC only on the specific endpoint
# but the list/watch endpoint returns all resources
```
Also check: search endpoints, label-selector queries, and field-selector queries may bypass per-resource authorization.

### CSRF on K8s Management UIs

K8s management UIs (ArgoCD, Rancher, Dashboard, Lens) on subdomains are CSRF targets:
```bash
# If ArgoCD is at argocd.target.tld, test:
# 1. Does it accept cross-origin requests?
# 2. Is CSRF protection origin-aware or just site-aware?
# 3. Can a page on another subdomain trigger state-changing actions?
curl -X POST "https://argocd.target.tld/api/v1/applications" \
  -H "Origin: https://evil.target.tld" \
  -H "Content-Type: application/json" -d '{}'
```

### Half-Blind to Full SSRF Pivot in Cloud Controllers

SSRF in upstream K8s controller code becomes full SSRF in vendor-managed environments. Half-blind SSRF at `169.254.169.254` + known token path = credential theft. Test cloud controller manager endpoints for URL-accepting parameters.

### Namespace Label Squatting

If RBAC or NetworkPolicy selects by namespace label and you can update namespaces:
```bash
kubectl auth can-i update namespaces
kubectl label namespace attacker-ns team=platform --overwrite
# May grant NetworkPolicy access or RBAC permissions intended for the platform team
```

### Authority Coupling in Resource Path Parsing

When a parameter encodes multiple authority decisions (`namespace/name`, `org/repo`):
```bash
# Test path injection across authority boundaries
kubectl get pod "../../kube-system/coredns-xxxxx"
```
