---
description: Find WAF bypass techniques using parser differentials between Go (Coraza WAF) and Node.js (backend). Use when hunting for WAF bypasses in Coraza/Next.js environments. Usage: /waf-bypass-hunter. Requires environment on localhost:8009 (executor) and localhost:9091 (WAF).
---
...
Bypass the WAF to execute exploit payloads by finding parser differentials between Go (Coraza WAF) and Node.js (Next.js backend).

## Architecture

```
┌─────────┐    ┌─────────────┐    ┌─────────────────┐
│  Agent  │───▶│ Coraza WAF  │───▶│ Next.js Backend │
│         │    │ (Go)        │    │ (Node.js)       │
│         │    │ Port: 9091  │    │ Port: 3000      │
└─────────┘    └─────────────┘    └─────────────────┘
```
...
Study parser implementations in the repo's `sources/` directory (clone from https://github.com/HacktronAI/skills if needed):
