...
|---|---|---|
| CRLF stripping on `Location` header | Positional API variant bypasses sanitizer; `request.header.X.1` unsanitized while `request.header.X` is sanitized | Google Apigee header injection, $133.7K |
| CDN strips CRLF in request path | Bare CR (`\r`) after HTTP method — CDN ignores it but backend treats `GET\r` as unknown method, triggers 501 cached via negative caching | Google Cloud CDN cache poisoning, $500K |
| WAF blocks `%0d%0a` patterns | Unicode CRLF equivalents `%E5%98%8A%E5%98%8D` (U+560A/U+560D) normalize to CR/LF downstream | Twitter ton.twitter.com CRLF + XSS, $1.68K |
| Framework-level CRLF rejection (Node 4.6+, Python 3.5+) | SMTP config parameter injection — CRLF in hostname field reaches INI/YAML config parser, not HTTP stack | Grafana RCE via SMTP parameter, $5K |
| Proxy validates Host header against allowlist | `X-Forwarded-Host` bypasses Host validation; or HTTP request smuggling injects XFH in smuggled request | GitLab Jira OAuth SSRF, $4K; Basecamp HRS cache poison, $1.7K |
| Cloud Armor WAF path-match blocking | Chunk-extension BWS parser differential: `\r` inside BWS accepted by LB, treated as `\r\n` by Node.js backend | Google GCLB request smuggling, $600K |
| Apache/nginx strip CRLF in modern versions | HTTP/2 pseudo-header injection: space in `:method` creates injected request line on HTTP/1.1 downgrade | Apache mod_proxy H2 smuggling, $1.2K (CVE-2021-44224) |
| `Content-Disposition` filename quoting | Unescaped double-quote in filename closes `filename=""` early; browser saves truncated name (`.bat` not `.bat".png`) | Nextcloud Mail filename injection, $125 |
| Single Host header validation | Duplicate Host headers — proxy uses first, app uses second (or vice versa); or absolute URL in request line overrides Host | Shopify cache poisoning, $2.9K |

## Chain Patterns

| Base Finding | Chain With | Combined Impact | Real Example |
|---|---|---|---|
| CRLF in response header (Set-Cookie injection) | Cookie-scoped session fixation + file-extension confusion XSS | Attacker sets auth cookie on victim, victim fetches attacker-uploaded polyglot JPG/HTML as `.html`, stored XSS fires | Twitter ton.twitter.com, $1.68K |
| X-Forwarded-Host reflected in HTML body attributes | CDN caching response without XFH in cache key | Stored DOM XSS via cache poisoning — all visitors get attacker-controlled i18n URLs; JS fetches attacker payload | catalog.data.gov defacement, $750 |
| HTTP/2 pseudo-header injection (`:method` with space) | WAF/ACL only sees H2 `:path`, backend processes injected H1 request line | WAF bypass: access admin endpoints the WAF thought it was blocking | Apache mod_proxy H2 smuggling, $1.2K |
| CRLF in SMTP host config field | Config file injection (INI section, YAML alias) | Inject new config sections enabling dangerous features; escalate to Go template injection RCE | Grafana SMTP RCE, $5K |
| Positional API header injection (`request.header.X.1`) | Override `Authorization` or `X-Authenticated-User` on upstream request | Gateway-level auth bypass; attacker sends privileged requests through API gateway to internal services | Google Apigee, $133.7K |
| Chunk-extension BWS differential (bare `\r`) | Backend parses smuggled second request; Cloud Armor/WAF never inspected it | WAF bypass for all GCLB + Node.js customers; access to any "protected" backend path | Google GCLB smuggling, $600K |
| Host header controls SSR fetch base URL | SSRF to cloud metadata (169.254.169.254) via redirect-following | IAM credential theft from metadata endpoint; full cloud account takeover | Angular Universal SSR SSRF, $500K |
| `Content-Disposition` filename injection | Email gateway extension filter bypass (gateway sees `.png`, browser saves `.bat`) | Social engineering: victim downloads and opens "image" that is actually executable | Nextcloud Mail, $125 |
...
4. Smuggling lives at the boundary — identify the proxy to backend pair (CDN to origin, ingress to service) and target the framing disagreement.
...
8. When a product has multiple API methods for the same operation (set header by name vs. set header by position), test EVERY method independently. Sanitizers are bolted onto the most-used path; positional/indexed variants are often missed ($133.7K pattern).
9. For HTTP/2 targets, fuzz every pseudo-header (`:method`, `:path`, `:authority`, `:scheme`) with characters forbidden in HTTP/1.1 tokens: space, CRLF, NUL, tab. Each is an injection sink during H2-to-H1 downgrade.
10. RFC chunk-extension BWS only permits SP and HTAB. Any other byte (`\r`, NUL, extended ASCII) forwarded by the frontend but re-interpreted by the backend is a smuggling primitive. Test each byte systematically.
11. For managed multi-tenant services (Grafana, Elastic, Airflow), fuzz every customer-settable string with `\r\n`. When the value hits a config file parser (INI, YAML, dotenv), newline injection becomes section/key injection with RCE potential.
12. Always check `Content-Disposition` filename handling with quotes (`"`), backslash (`\`), semicolon (`;`), and Unicode RTL override in filenames. Browser parsers diverge from server-side display, creating trust mismatches.

## Summary

Header injection is fundamentally a normalization failure: somewhere on the request to response path, user input reached a header value without CR/LF stripping or proper escaping. The impact tiers up from open redirect to cache poisoning to request smuggling depending on which downstream component trusts the resulting header. The highest payouts ($133K-$600K) come from API method variant bypasses and parser differentials at proxy-to-backend boundaries. Audit every header whose value moves with input, and treat every `X-Forwarded-*` / Host trust as a security boundary that needs explicit justification.
