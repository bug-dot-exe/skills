...
```bash
mkdir -p /workspace/.bugdotexe-source-aware
```
...
```bash
ART=/workspace/.bugdotexe-source-aware
mkdir -p "$ART"
...
art = Path("/workspace/.bugdotexe-source-aware")
...
semgrep scan --config p/default --config p/golang --config p/secrets \
  --metrics=off --json --output /workspace/.bugdotexe-source-aware/semgrep.json .

# If you choose auto config, do not combine it with --metrics=off
semgrep scan --config auto --json --output /workspace/.bugdotexe-source-aware/semgrep-auto.json .
```
...
xargs -r -n 200 sg run --pattern '$F($$$ARGS)' --json=stream \
  < /workspace/.bugdotexe-source-aware/sg-targets.txt \
  > /workspace/.bugdotexe-source-aware/ast-grep.json 2> /workspace/.bugdotexe-source-aware/ast-grep.log || true
```
...
```bash
gitleaks detect --source . --report-format json --report-path /workspace/.bugdotexe-source-aware/gitleaks.json
trufflehog filesystem --json . > /workspace/.bugdotexe-source-aware/trufflehog.json
```
...
```bash
trivy fs --scanners vuln,misconfig --timeout 30m --offline-scan \
  --format json --output /workspace/.bugdotexe-source-aware/trivy-fs.json . || true
```
