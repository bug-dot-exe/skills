...
CSPT is the client-side analog of SSRF. Any URL-bar parameter that becomes a path segment in a same-origin API call is a candidate:
1. Identify all client-side `fetch()`/`XMLHttpRequest` calls that incorporate URL parameters into the request path
2. Test path traversal sequences (`../`) in those parameters to redirect the client-side API call to a different endpoint
3. When combined with a same-origin API that performs sensitive actions, CSPT becomes a full exploit chain
4. Next.js dynamic route segments (`[slug]`, `[...catchAll]`) are prime candidates -- they flow directly into client fetch paths

### SSR Framework SSRF
Next.js SSR/ISR features that automatically resolve URLs are an SSRF supply chain:
- `getServerSideProps` or Server Components fetching user-controlled URLs during SSR
- `next/image` with overly broad `remotePatterns`: test internal hosts, IPv4/IPv6 variants, DNS rebinding through the image optimizer
- Import-from-URL features (file paste, data import, link preview) where the server fetches user-supplied URLs
- Framework convenience features (automatic URL handling, API proxying) that silently perform server-side fetches

### Metadata SSRF on Cloud Deployments
Next.js deployed on GCP/AWS/Azure cloud with any server-side URL fetch:
- Test `http://169.254.169.254/`, `http://metadata.google.internal/`, `http://metadata/`
- Cloud metadata endpoints yield service account tokens, project IDs, secrets
- SSRF-via-import-URL: find any feature accepting a URL parameter that the server fetches -- submit cloud metadata URLs
- Half-blind to full pivot: upstream SSRF findings in open-source code gain impact when paired with the specific cloud provider's metadata endpoints

### Redirect-Follow SSRF Chain
For every outbound HTTP feature in a Next.js app:
1. Test direct internal-URL submission -- usually blocked by allowlists
2. Test DNS rebinding: domain that resolves to internal IP after initial resolution
3. Test open redirect chaining: find an open redirect on an allowed domain, use it to bounce to internal targets
4. Test protocol downgrade: HTTPS allowlist but HTTP redirect to internal service
5. Filter-vs-resolver disagreement: string-level validator and separate resolution library parsing URLs differently

### Parser-Differential Path Confusion
Between Next.js middleware, load balancer, and route handlers:
- Send unusual characters in URL paths (`@`, `\`, `;`, `..`, `%00`, `%2f`, leading `-`)
- Monitor for routing-layer error responses revealing backend technology stack
- Double-encoding (`%252f`) may be decoded differently by middleware vs handler
- Path confusion between Next.js middleware normalization and upstream CDN/proxy routing rules
- `x-middleware-subrequest` and `x-nextjs-data` headers can alter routing behavior

### Document Rendering Pipeline SSRF
Any Next.js feature generating PDFs, exports, or print views:
- PDF generation libraries (Puppeteer, wkhtmltopdf, Prince) execute JavaScript in the rendered HTML
- Inject `<img src="http://internal:8080/">` or `<link href="http://metadata/">` in content that gets rendered to PDF
- Markdown/HTML rendering during SSR that fetches external resources
- Preview/embed features that render user-supplied URLs server-side

### Webhook and Integration SSRF
Next.js SaaS applications with webhook or integration features:
- Map every feature where the user supplies a URL the backend will fetch (webhooks, integrations, import, embed)
- Test each against internal network ranges, cloud metadata, and localhost services
- Webhook signature verification may be optional or bypassable
- SaaS tool integrations (Sentry, Datadog, Segment) with server-side features accepting attacker-controlled URLs from the client
