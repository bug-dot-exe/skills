...
- **SDK URL-construction injection:** If client code embeds user IDs into Supabase SDK calls, test `.`, `..`, `/`, `//`, `%2e%2e` as ID values — SDK path-join flaws can traverse to other tables/buckets
- **Email artifact audit:** Every Supabase auth email (reset, confirm, magic link) embeds URLs — verify HTTPS enforcement, token entropy, and Referer leakage to third-party scripts on the landing page
- **Validate-and-use mismatch on signed URLs:** Storage signed URLs may be validated by one service and consumed by another — test if the signed path and the fetched path are parsed identically (encoding differences, double-decode, trailing dots)
- **Feature-as-SSRF inventory:** Edge Functions with `fetch()` are SSRF candidates — enumerate every function that accepts a URL parameter and test metadata endpoints (`169.254.169.254`, `metadata.google.internal`)
- **Numeric flag flipping in auth flows:** For Supabase auth parameters (`type`, `flow`, `provider`), test `0`→`1`, `false`→`true`, and alternate numeric values to toggle hidden behavior paths
